...
export default function CineMatch() {
  ...
  const searchMovies = async () => {
    ...
    setLoading(true);
    const url = `${BASE_URL}/search/${mediaType}?api_key=${API_KEY}&query=${encodeURIComponent(
      searchQuery
    )}`;
            ...
            <div className="bg-slate-800 rounded-2xl p-6 shadow-2xl">
              ...
              {watchedMovies.length === 0 ? (
                <p className="text-center text-gray-400 py-12">
                  Your library is empty. Start adding movies from the Discover
                  tab!
                </p>
              ) : (
                <div className="flex gap-4 overflow-x-auto pb-4">
                  {watchedMovies.map((movie) => (
                    <div key={movie.id} className="flex-shrink-0">
                      ...
                      <p className="mt-2 text-sm text-center truncate w-32">
                        {movie.title}
                      </p>
                    </div>
                  ))}
